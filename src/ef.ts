...
export class DBSchema {
    public static create (dbName: string, dbVersion: number, schema: Object);
    public static create (jsonFilePath:string);     
    public static create (...args: any[]){//(dbName: string, dbVersion: number, schema: Object) {       
...
export class DBContext<T_CTX> {
    ...
    public transaction( fn: (tx: lf.Transaction, context: T_CTX)=>Promise<any>) : Promise<any>{
        ...
        return this.context.tx.begin(this.context.tables).then(()=>{
            var p= fn(this.context.tx,<T_CTX>this.context.tableSchemaMap).then(()=>{
    ...
    public get tables() : T_CTX{
        return <T_CTX>this.context.tableSchemaMap;
    }
    ...
    public DBEntity<T, T_CTX>( tableName:string, navigationProperties?: string[] ){                
        return <DBEntity<T, T_CTX>>(new DBEntityInternal<T, T_CTX>(this.context, tableName, navigationProperties, this.ready));
    }          
}
...
class DBEntityInternal<T, T_CTX> implements DBEntity<T, T_CTX> {
